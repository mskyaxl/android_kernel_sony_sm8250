...
static void mtp_complete_out(struct usb_ep *ep, struct usb_request *req)
{
	...
	dev->rx_done = 1;
	if (req->status != 0 && dev->state != STATE_OFFLINE && dev->state != STATE_BUSY)
		dev->state = STATE_ERROR;
	...
	size_t count, loff_t *pos)
{
	...
	if (dev->state == STATE_CANCELED) {
		...
		dev->state = STATE_READY;
		spin_unlock_irq(&dev->lock);
		mtp_log("mtp_read DEVICE RESET. State: %d.\n", dev->state);
		return -ECONNRESET;
	}
	...
	while (count > 0 || write_req) {
		...
		if (read_req) {
			...
			if (dev->state == STATE_RESET) {
				mtp_log("%s: DEVICE RESET\n", __func__);
				r = -ECONNRESET;
				if (!dev->rx_done) {
					mtp_log("dequeue in DEVICE RESET\n");
					usb_ep_dequeue(dev->ep_out, read_req);
				}
				break;
			}
	...
	struct mtp_file_range *mfr)
{
	...
	if (dev->state == STATE_RESET) {
		...
		mtp_log("report reset to user space\n");
		dev->state = STATE_READY;
		spin_unlock_irq(&dev->lock);
		ret = -ECONNRESET;
		goto out;
				...
				list_for_each_entry(f, &cfg->functions, list) {
					...
					if (!strncmp(f->name, "mtp", 3)) {
						memcpy(func->compatibleID,
							"MTP", 3);
						mtp_log("MTP interface found. Interface_num: %d.\n",
							interface_num);
					}
					func++;
					func_num++;
